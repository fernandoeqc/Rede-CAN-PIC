...
void lcd_send_byte(unsigned int address, unsigned int n)
{
   ...
   lcd_output_rs(0);
   //while ( bit_test(lcd_read_byte(),7) ) ;    /////////////TRAVA O LCD_INIT ///////// PESQUISAR DEPOIS
...
void lcd_putc(char c)
{
   switch (c)
   {
     ...
     #if defined(LCD_EXTENDED_NEWLINE)
      case '\r'   :  lcd_gotoxy(0, ~g_LcdY);   break;
      case '\n'   :
     ...
     #if defined(LCD_EXTENDED_NEWLINE)
      default     : 
         if (g_LcdX < LCD_LINE_LENGTH) g_LcdX++;
         else{
            g_LcdX = 0;
            g_LcdY = ~g_LcdY;
            lcd_gotoxy(0,g_LcdY);
...
void lcd_set_cgram_char(unsigned int which, unsigned int *ptr)
{
   ...
   for(i=0; i<8; i++) lcd_send_byte(1, *ptr++);
 
  
   #if defined(LCD_EXTENDED_NEWLINE)
    lcd_gotoxy(g_LcdX+1, ~g_LcdY);  //set ddram address
   #endif
}

void lcd_cursor_on(int1 on)
{
   if (on) lcd_send_byte(0,0x0F);           //turn LCD cursor ON
...
void lcd_putc_hexa(unsigned int32 hexa){
   ...
   for(i = 1; i <= 4; i++)
   {
     if(hexa_temp) tamanho = i;
     hexa_temp = hexa_temp >> 4;   
   }
